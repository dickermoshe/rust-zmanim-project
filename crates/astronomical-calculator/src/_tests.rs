...
//                 // set to noon
//                 let dt = dt.with_hour(12).unwrap();

//                 let our_result = AstronomicalCalculator::standard(dt, longitude, latitude, elevation)
//                 .map(|inputs| inputs.calculate_solar_position())
//                 .ok();
//         let result = SunriseSunsetParameters::new((dt.timestamp_millis() / 1000) as i64, latitude, longitude).calculate();
//     println!("dt: {:?}", dt);
...
//                         let rise = timezone.timestamp_opt(result.rise,0).single().unwrap();

//                         println!("rise: {:?}", rise);
//                         println!("our sunrise: {:?}", sunrise_time);
//                         let diff = sunrise_time - rise;
//                         prop_assert!(diff.num_seconds() < 1800, "rise and set should be within 1800 seconds of each other");
