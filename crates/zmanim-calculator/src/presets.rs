...
pub enum ZmanPreset<'a, Zman: ZmanLike<Tz>, Tz: TimeZone> {
    Event(
        ZmanPrimitive<'a>,
        #[cfg(feature = "_java_testing")] &'static str,
        PhantomData<Tz>,
    ),
    Custom(Zman, #[cfg(feature = "_java_testing")] &'static str),
}

impl<'a, Zman: ZmanLike<Tz>, Tz: TimeZone> ZmanPreset<'a, Zman, Tz> {
    pub fn event(event: ZmanPrimitive<'a>, name: &'static str) -> Self {
        #[cfg(feature = "_java_testing")]
        return Self::Event(event, name, PhantomData);
        #[cfg(not(feature = "_java_testing"))]
        return Self::Custom(event, PhantomData);
    }
    pub fn custom(z: Zman, name: &'static str) -> Self {
        #[cfg(feature = "_java_testing")]
        return Self::Custom(z, name);
        #[cfg(not(feature = "_java_testing"))]
        return Self::Custom(z, name);
    }
}
impl<'a, Zman: ZmanLike<Tz>, Tz: TimeZone> ZmanLike<Tz> for ZmanPreset<'a, Zman, Tz> {
    fn calculate(
        &self,
        calculator: &mut ZmanimCalculator<Tz>,
    ) -> Result<DateTime<Utc>, ZmanimError> {
        match self {
            Self::Event(event, name, _) => event.calculate(calculator),
            Self::Custom(event, name) => event.calculate(calculator),
        }
    }
}
